...
def getSub(train_data, test_data, train_label, dataset_name):
    ...
    feaSubsets = XGB_RF_AUC_only_train.XGB_RF_ADB_feature_selection(train_FS, 'RF', dataset_name)

    order = [i + 1 for i in feaSubsets]

    train_data_sub_RF = TRAIN_DATA[:, feaSubsets]
    test_data_sub_RF = TEST_DATA[:, feaSubsets]

    return train_data_sub_RF, test_data_sub_RF, np.array(order).reshape((1, -1))