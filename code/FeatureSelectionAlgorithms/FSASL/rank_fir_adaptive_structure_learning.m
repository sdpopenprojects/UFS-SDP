function rankx = rank_fir_adaptive_structure_learning(X, nClass, options)
% -----------------------------------------------------------------------
% Please refer to
%  Du, L. and Shen, Y.D., 2015, August. Unsupervised feature selection 
%       with adaptive structure learning. In Proceedings of the 21th 
%           ACM SIGKDD international conference on knowledge discovery 
%               and data mining (pp. 209-218). ACM.
% -----------------------------------------------------------------------
if nargin < 2
    nClass = 2;
end
...
for iter = 1:options.maxiter
    ...
    if options.lambda2 > 0
        ...
        A = (A+A')/2;
        LL = diag(sum(A)) - A;%计算局部结构
        LL = (LL + LL') / 2;
    else
        LL = 0;
    end
    
    L = options.lambda1 * LG + options.lambda2 * LL;%组合
    ...
    X2 = W' * X;%更新嵌入矩阵 X2，这是特征选择的结果，用于下一次迭代
    obj = trace(X2 * L * X2');
...
[ ~, rankx ] = sort( abs( W(:,1) ) + abs( W(:,2) ), 'ascend' );
rankx = rankx';
end
