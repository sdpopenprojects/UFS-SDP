...
%         end
        ...
        for ii=1:n
            ...
            W(:,ii) = C\ones(k,1);                           % solve Cw=1  有NAN值  A\B = inv(A)*B
            %W(:,ii) = pinv(C)*ones(k,1);
            sum(W(:,ii));%有NAN值
            %在Matlab中，sum函数可以用来计算矩阵或向量中所有元素的和。但是，如果矩阵或向量中有NaN（非数字）值，那么sum函数就会出现错误，返回NaN。
            % 解决方法：可以使用Matlab中的nansum函数来解决这个问题，nansum函数可以忽略NaN值，并计算矩阵或向量中其他元素的和。
