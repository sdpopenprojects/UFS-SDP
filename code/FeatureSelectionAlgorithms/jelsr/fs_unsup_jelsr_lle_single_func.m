% function [feaSubsets,res_gs,res_aio, res_gs_ps] = fs_unsup_jelsr_lle_single_func(dataset, exp_settings, ~)
% function [feaSubsets,res_gs,res_aio, res_gs_ps] = fs_unsup_jelsr_lle_single_func(X, Y, exp_settings, ~)
function [feaSubsets] = fs_unsup_jelsr_lle_single_func(X, exp_settings, ~)
...
parfor i1 = 1:length(paramCell)
    ...
    param = paramCell{i1};
    [~, W] = computeLocalStructure(X, param.weightMode, param.k, param.t);%W里有nan值 构建局部结构，计算相邻性
...
%save(fullfile(prefix_mdcs, [dataset, '_best_result_JELSR_lle.mat']),'FeaNumCandi','res_gs','res_aio', 'res_gs_ps');
end